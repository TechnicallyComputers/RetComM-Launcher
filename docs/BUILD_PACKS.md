...
`recomp-ui/` + emitters at the release pins). Otherwise Retro falls back to the
GitHub source zipball at `build.source.ref` (note: zipballs omit git submodules).

**Shared engines:** after extract/sync, Retro harvests each vendored framework
...
Update works without Developer Mode). On Windows, junction targets are passed as
normal DOS paths (not `\\?\…`) and the per-title tree is parked/restored if
linking fails — so a failed `mklink` never leaves Install without `psxrecomp`.
...
Retro harvests CLI + `psxrecomp-game` / `psxrecomp-bios` into the shared SDK
...
are also skipped during sync — Retro re-harvests them from the staging zip
...
**Codegen reuse on update:** after the first successful generate, Retro keeps
...
**CMake configure reuse:** after a successful `cmake -S/-B`, Retro writes
...
**Prune shared caches now** / `retcomm cache gc`), Retro drops old
...
cmake-clang-v1-<os>/
  ...
  bin/ccache         # optional; Retro enables CMAKE_*_COMPILER_LAUNCHER when present
...
Retro prepends `<pack>/bin` (or the single nested folder’s `bin/`) to `PATH`
...
`cmake-clang-v1` from
[RetroPortingToolKit/RetroPorting-Toolchains](https://github.com/RetroPortingToolKit/RetroPorting-Toolchains)
...
- Toolchain release assets: build in **RetroPorting-Toolchains** via its CI
